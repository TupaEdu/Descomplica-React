...
const ListarTarefa = () => {
  ...
  // };
  const handleDeletar = (id) => {
    const tarefa = tarefas.find((tarefa) => tarefa.idTarefa === id);
  ...
  const confirmDeletar = () => {
    setTarefas(current =>
      current.filter(tarefa => {
        return tarefa.idTarefa !== tarefaParaDeletar.idTarefa;
      }),
    );
    ...
    >
      <Box sx={{ ...style, width: 400 }}>
        ...
        <Typography id="confirm-modal-description" sx={{ mt: 2 }}>
          Tem certeza que deseja excluir a tarefa "{tarefaParaDeletar?.tituloTarefa}"?
        </Typography>
        <Button onClick={confirmDeletar} variant="contained" color="error" sx={{ mt: 2 }}>
          Confirmar
        </Button>
        <Button onClick={() => setOpenConfirm(false)} variant="outlined" sx={{ mt: 2, ml: 2 }}>
          Cancelar
        </Button>
      </Box>
    </Modal>
